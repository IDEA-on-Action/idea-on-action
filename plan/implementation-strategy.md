...
## 🔄 사이트 재구조화 병렬 진행 전략 (2025-11-23)

### 병렬 진행 개요

Sprint 2와 Sprint 3는 서로 독립적이므로 동시에 병렬 진행 가능합니다.
...
Sprint 1 (완료) ✅
     ...
     ├─────────────────┬─────────────────┐
     ▼                 ▼                 ▼
Sprint 2          Sprint 3          CMS Phase 1
(Agent 1, 2)      (Agent 3, 4)      (Agent 5)
     │                 │                 │
     └─────────┬───────┘                 │
               ▼                         ▼
          Sprint 4                   CMS Phase 2
        (Week 3-4)                   (Week 4-5)
```

### 에이전트 할당

#### Sprint 2: 프로젝트 페이지 통합 (Agent 1, 2)
| Day | Agent 1 | Agent 2 |
|-----|---------|---------|
| 1 | TASK-010: ProjectsHub 완성 (3h) | TASK-012: ProjectCard 확장 (2h) |
| 2 | TASK-011: 탭 컴포넌트 추출 (4h) | TASK-013: ProjectDetail 생성 (3h) |
| 3 | TASK-014: 훅 연동 검증 (1h) | 버그 수정 |
| 4 | TASK-015: E2E 테스트 (2h) | 리팩토링 |

#### Sprint 3: 이야기 & 함께하기 (Agent 3, 4, 5)
| Day | Agent 3 | Agent 4 | Agent 5 |
|-----|---------|---------|---------|
| 1 | TASK-016: StoriesHub (2h) | TASK-034: ConnectHub (1h) | TASK-018, 021: DB 마이그레이션 (2h) |
| 2 | TASK-017: Changelog (3h) | TASK-035~037: Connect 하위 (3h) | TASK-022: 훅 생성 (2h) |
| 3 | TASK-023, 024: 이전 (1h) | TASK-019, 020: Newsletter (3h) | 통합 테스트 |
| 4 | TASK-025: E2E 테스트 (2h) | 버그 수정 | 리팩토링 |

### 예상 시간 절감

| Sprint | 순차 실행 | 병렬 실행 | 절감률 |
|--------|----------|----------|--------|
| Sprint 2 | ~15시간 | ~9시간 | 40% |
| Sprint 3 | ~19시간 | ~9시간 | 53% |
| **합계** | ~34시간 | ~9시간 | **74%** |

### 의존성 관리

#### 선행 작업 (Sprint 1 완료 필수)
- Header.tsx 5개 메뉴 구조
- App.tsx 리디렉션 라우트
- 허브 페이지 스켈레톤

#### 병렬 가능 작업
- Sprint 2 컴포넌트 (TASK-010~015)
- Sprint 3 컴포넌트 (TASK-016~037)
- DB 마이그레이션 (TASK-018, 021)

#### 순차 필수 작업
- TASK-011 → TASK-014 (탭 → 훅 검증)
- TASK-018 → TASK-017 (DB → Changelog)
- TASK-021 → TASK-019 (DB → Newsletter)

### 검증 체크포인트

#### Day 2 종료 시
- [ ] ProjectsHub 탭 전환 동작
- [ ] StoriesHub 4개 섹션 렌더링
- [ ] ConnectHub 3개 카드 렌더링
- [ ] DB 마이그레이션 로컬 테스트
